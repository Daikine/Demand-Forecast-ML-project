...
def main(args):
    ...
    for sku, (X, y, dates) in data.items():
        ...
        # Обучаемся в стандартизированном log-space. WAPE в этом пространстве нельзя
        # использовать как loss: после StandardScaler сумма target может быть близкой к нулю,
        # что делает градиенты нестабильными. Валидацию ниже считаем в исходном масштабе.
        ...
        for epoch in range(args.epochs):
            ...
            # ранняя остановка по WAPE, если модель не улучшается
        ...
        with torch.no_grad():
            for xb_p, xb_f, yb in val_loader:
                ...
                yt = np.maximum(from_log_target(targ_scaler.inverse_transform(yb.numpy().reshape(-1, 1)).reshape(yb.shape)), 0.0)
                val_pred_cal.append(yp)
                val_true_cal.append(yt)

        val_pred_cal = np.concatenate(val_pred_cal) if val_pred_cal else np.empty((0, args.horizon))
        val_true_cal = np.concatenate(val_true_cal) if val_true_cal else np.empty((0, args.horizon))
        if len(val_pred_cal):
            scale = np.sum(val_true_cal) / np.maximum(np.sum(val_pred_cal), 1e-6)
        ...
        yp_all = np.maximum(yp_all * scale + bias, 0.0)
